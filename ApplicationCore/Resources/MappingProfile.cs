  ...
  public class MappingProfile : Profile {
    public MappingProfile() {
      ...
      CreateMap<Product, AddProductResource>()
          .ForMember(p => p.ManufacturerId,
                     opt => opt.MapFrom(pr => pr.Manufacturer.ManufacturerId))
          .ForMember(
              p => p.MeasurementUnitId,
              opt => opt.MapFrom(pr => pr.MeasurementUnit.MeasurementUnitId))
          .ForMember(p => p.QuantityAtWarehouse,
                     opt => opt.MapFrom(pr => pr.QuantityAtWarehouse))
          .ReverseMap();
    }
  }
}
